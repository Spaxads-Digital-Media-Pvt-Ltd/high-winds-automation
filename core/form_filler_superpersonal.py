...
class FormFiller:
    ...
    def _fill_form(self, page: Page, f: dict, row_number: int, stop_event=None) -> None:
        ...
        for step_num in range(0, 60):
            ...
            # reset/cycled — fail fast instead of silently burning all 60 steps.
    ...
    def _handle_step(self, frame: Frame, title: str, f: dict) -> str | None:
        ...
        if (
            ...
            if clicked:
                time.sleep(0.8)
                # LCF pay-frequency chips auto-advance on selection. Only click
                # Continue if present AND enabled; never force the next step.
                self._click_primary_continue(frame)
                return clicked
            ...
            for label in (
                ...
                if label and (
                    ...
                    return label
            if not self._strict_sheet:
                return self._chip(frame, "Bi-Weekly (Every 2 Weeks)")
            return None
    ...
    def _site_profile(self, url: str) -> str:
        # This module exclusively drives the Super Personal Finder funnel, now
        # reached via a tracker redirect (digipalz.trackog.net -> superpersonalfinder.com),
        # so the target URL no longer contains "superpersonalfinder". Default to
        # the SPF profile unless an explicit override is configured.
        explicit = (self._target.get("site") or "").strip().lower()
        if explicit in {"borrowmoney", "borrow_money", "bm"}:
            return "borrowmoney"
        if explicit == "default":
            return "default"
        if "borrowmoney" in (url or "").lower():
            return "borrowmoney"
    ...
    def _prepare_entry_page(
        ...
        if site == "superpersonalfinder":
            ...
            if "iframe.global" in frame_urls:
