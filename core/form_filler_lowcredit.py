...
class FormFiller:
    ...
    def _fill_form(self, page: Page, f: dict, row_number: int, stop_event=None) -> None:
        ...
        time.sleep(3)  # let the SPA fully render before polling
    ...
    def _get_frame(self, page: Page) -> Frame:
        ...
        for i in range(30):
            ...
            if frames:
                return frames[0]
            # lowcreditfinance.com usually embeds the iframe.global form on load,
            # but sometimes it only appears after clicking "Request Funds". If it
            # hasn't embedded after a few seconds, click that CTA to trigger it.
            if i >= 3 and not clicked_cta:
                try:
                    page.evaluate(
                        """() => {
                            const b = [...document.querySelectorAll('a,button,[role=button]')]
                                .find(e => e.offsetParent !== null && /request funds/i.test(e.textContent || ''));
                            if (b) b.click();
                        }"""
                    )
                    clicked_cta = True
    ...
    def _map_loan_purpose(self, raw: str) -> str:
        raw = raw.lower().strip()
        # Live LowCreditFinance purpose chips (verified against the form):
        # Emergency Purchase | Debt Consolidation | Medical Bill |
        # Car Repair | Home Improvement | Other
        if not raw:
            return "Debt Consolidation"
        if "debt" in raw or "consol" in raw:
            return "Debt Consolidation"
        if "medical" in raw or "health" in raw or "dental" in raw:
            return "Medical Bill"
        if "auto" in raw or "car" in raw or "vehicle" in raw or "repair" in raw:
            return "Car Repair"
        if "home" in raw or "house" in raw or "improv" in raw or "renovat" in raw:
            return "Home Improvement"
        if "emergency" in raw or "urgent" in raw:
